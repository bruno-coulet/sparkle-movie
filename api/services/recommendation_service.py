...
class RecommendationService:
    ...
    def get_user_ratings(user_id: int, limit: int = 50) -> dict[str, Any]:
        ...
        user_avis = (
            ...
            .select(col("movieId"), col("title"), col("rating"), col("timestamp"))
        ...
        return {"user_id": user_id, "total_avis": len(avis), "avis": avis}
...
class StatisticsService:
    ...
    def get_genres() -> dict[str, list]:
        ...
        genres_raw = (
            movies_df.select(explode(split(col("genres"), r"\|")).alias("genre"))
            .groupBy("genre")
            .count()
            .orderBy(desc("count"))
        )
